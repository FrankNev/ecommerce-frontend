  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      {stats.map(({ label, value, icon: Icon }) => (
        <div key={label} className="bg-white ring-1 ring-foreground/10 border border-gray-100 rounded-xl p-4 flex items-center gap-4">
          <div className="w-9 h-9 bg-gray-100 rounded-xl flex items-center justify-center shrink-0">
            <Icon size={16} className="text-gray-500" />
          </div>
          <div>
            <p className="text-sm md:text-xs font-medium md:font-normal text-gray-500">{label}</p>
